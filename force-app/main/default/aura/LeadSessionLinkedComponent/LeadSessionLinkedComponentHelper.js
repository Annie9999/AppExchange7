({
    getLeadSessionLiked: function (component, event, helper) {
        ...
        //component.set("v.recordId",'a002w000004kyzFAAQ');
   ...
   /* getProjectDetail: function (component, event, helper, projectId) {
    ...
    getLeadPicklist: function (component, event, helper) {
        ...
        action.setCallback(this, function (response) {
            ...
            if (state == 'SUCCESS') {
                var rtnValue = response.getReturnValue();
                console.log(rtnValue);
                console.log(' #LeadSource: '+rtnValue.picklistLeadSource);
    ...
    getSocialIdData: function (component, event, helper) {
        ...
        action.setCallback(this, function (response) {
            ...
            if (state == 'SUCCESS') {
                var rtnValue = response.getReturnValue();
                console.log(rtnValue);
                console.log(' #socialID: '+rtnValue.socialID);
        ...
        for (var idx in records) {
            if (leadDataSession.length > 0) {
                for (var index in leadDataSession) {
                    ...
                    if (records[idx].Id != leadDataSession[index].iigproduct__Lead__c) {
    ...
    //         leadEdit.iigproduct__Lead__r.Interested_Project__c = component.get("v.selectedProject").Id;
    //     } else {
    //         leadEdit.iigproduct__Lead__r.Interested_Project__c = '';
    //     }
    ...
    removeLeadToLeadSessionLinkage: function (component, event, helper, index) {
        ...
        console.log(' #removeindex2: '+index);
    ...
    //                         console.log(sessionLeadDup[index].iigproduct__ChatSession__r.CreatedDate);

    //                         var day = new Date(sessionLeadDup[index].iigproduct__ChatSession__r.CreatedDate).getDate();
    //                         var month = new Date(sessionLeadDup[index].iigproduct__ChatSession__r.CreatedDate).getMonth() + 1;
    //                         var year = new Date(sessionLeadDup[index].iigproduct__ChatSession__r.CreatedDate).getFullYear();
    //                         var time = new Date(sessionLeadDup[index].iigproduct__ChatSession__r.CreatedDate).toLocaleTimeString('nb-NO');
    //                         var CreatedDateDisplay = day + '/' + month + '/' + year + ' ' + time;
        ...
        action.setCallback(this, function (response) {
            ...
            if (state == 'SUCCESS') {
                ...
                if (socialAccount != undefined) {
                    component.set("v.leadNew.FirstName", socialAccount.iigproduct__FirstName__c);
                    component.set("v.leadNew.LastName", socialAccount.iigproduct__LastName__c);
                    component.set("v.leadNew.Email", socialAccount.iigproduct__Email__c);
                    component.set("v.leadNew.MobilePhone", socialAccount.iigproduct__Phone_Number__c);
    ...
    createLead: function (component, event, helper) {
       ...
       /* if (component.get("v.selectedProject") != null) {
        ...
        leadNew.iigproduct__FromChatSession__c = component.get("v.recordId");
        console.log('FromChatSession: '+leadNew.iigproduct__FromChatSession__c);
    ...
    resetModelCreateLead: function (component, Event, helper) {
       ...
       /* component.set("v.leadNew.Media_info__c", '');
        ...
        // component.set("v.mediaVal", '');
        // component.set("v.mediaInput", '');
        // component.set("v.leadSourceVal", '');
        // component.set("v.purposeVal", '');
    ...
    createLeadSessionLinkage: function (component, event, helper,LeadId) {
        console.log('createLeadSessionLinkage');
